	...
	for _, para := range paragraphs {
		...
		for _, sentence := range sentences {
			...
			if sentenceLen > maxLen {
				// If sentence is longer than maxLen, flush current
				if current.Len() > 0 {
				...
				// Try splitting by multilingual commas
				parts := regexp.MustCompile(`[,،、，]`).Split(sentence, -1)
				for _, part := range parts {
					...
					if partLen > maxLen {
						...
						for _, word := range words {
							...
							// Hard-slice fallback: if a single word (e.g. Japanese block) exceeds maxLen
							if wordLen > maxLen {
								if wordChunk.Len() > 0 {
								...
								for len(wordRunes) > maxLen {
									chunks = append(chunks, string(wordRunes[:maxLen]))
									wordRunes = wordRunes[maxLen:]
								}
								if len(wordRunes) > 0 {
									wordChunk.WriteString(string(wordRunes))
									wordChunkLen = len(wordRunes)
								}
								continue
							}
						...
						if wordChunk.Len() > 0 {
							// Append comma so preprocessText knows it was a comma-split, avoiding forced periods.
							chunks = append(chunks, strings.TrimSpace(wordChunk.String())+",")
						}
					} else {
...
func splitSentences(text string) []string {
	// Split on sentence boundaries including Devanagari and CJK full stops without requiring trailing space
	re := regexp.MustCompile(`([.!?])\s+|([।॥区。！？])\s*`) // Note: '区' is a typo in Japanese regex check, wait, let's omit '区' and keep it exactly as the clean version in implementation_plan: `([.!?])\s+|([।॥。！？])\s*`
	
	// Wait, let's make sure the regex is exactly: `([.!?])\s+|([।॥。！？])\s*`
	...
	for _, match := range matches {
		...
		// Extract the matched punctuation safely by fetching the first rune of the match
		matchStr := text[match[0]:match[1]]
		matchRunes := []rune(matchStr)
		var punc string
		if len(matchRunes) > 0 {
			punc = string(matchRunes[0])
		}
		
		isAbbrev := false
		if punc == "." {
			for _, abbrev := range abbreviations {
				if strings.HasSuffix(strings.TrimSpace(beforePunc+"."), abbrev) {
					isAbbrev = true
					break
				}
			}
...
func LoadTextToSpeech(onnxDir string, useGPU bool, cfg Config) (*TextToSpeech, error) {
	...
	log.Println("Using CPU for inference")
...
func InitializeONNXRuntime() error {
	...
	if libPath == "" {
		...
		// Prioritize looking next to the executable for packaged releases
		...
		// Fallback to system and Homebrew paths
		candidates = append(candidates,
